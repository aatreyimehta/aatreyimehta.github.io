...
#importing source data functions, channelwise target functions and grayscale target functions for 512x512, 1024x512, 760x512 image dimensions from module bar_generator_vertical
from bar_generator_vertical import generate_data, write_source_data, write_circle_target_data, write_circle_target_data1, write_source_data_elongated, write_circle_target_data_elongated, write_circle_target_data1_elongated, write_source_data_1, write_circle_target_data_1, write_circle_target_data1_1, serialize_data
from utils import postprocessing, maskgen   #importing postprocessing and maskgen from module utils
...
with tf.device('/device:GPU:0'):
    ...
    for i in tqdm(range(len(data)), desc='bar charts: '):
        ...
        fig_size = random.choices([[512,512], [512,512], [512,512]], weights=[.5, .25, .25])[0]   #initializing figure size 512x512(1:1 ratio)
        fig_size_elongated = random.choices([[1024,512], [1024,512], [1024,512]], weights=[.5, .25, .25])[0]   #initializing figure size 1024x512(2:1 ratio)
        fig_size_1 = random.choices([[760,512], [760,512], [760,512]], weights=[.25, .5, .5])[0]   #initializing figure size 760x512(1.5:1 ratio)

        draw_grid = random.random() < P_grid
        tick_step = random.randint(10, 16)

        #calling write_source_data() from bar_generator_vertical module
        ...
        #calling write_target_data() from bar_generator_vertical module
        ...
        #calling write_target_data1() from bar_generator_vertical module
        ...
        #similarly, calling methods from bar_generator_vertical module for other two image dimensions
        ...
        write_circle_target_data1_1(circle_data[i], f"./data/tactile1/t_{i+2001}.png", fig_size_1, draw_grid, tick_step)
