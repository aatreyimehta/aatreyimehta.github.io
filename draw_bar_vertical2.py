#importing python packages to begin withimport os
import numpy as np
import random
import pandas as pd
...
#importing source data functions, channelwise target functions and grayscale target functions for 512x1024, 512x760 image dimensions from module bar_generator_vertical2
...
P_grid = 0.8   #grid probability set as 80%
...
with tf.device('/device:GPU:0'):
    ...
    for i in tqdm(range(len(data)), desc='bar charts: '):
        ...
        fig_size_2 = random.choices([[512,1024], [512,1024], [512,1024]], weights=[.5, .25, .25])[0]   #initializing figure size 512x1024(1:2 ratio)
        fig_size_3 = random.choices([[512,760], [512,760], [512,760]], weights=[.5, .25, .25])[0]   #initializing figure size 512x760(1:1.5 ratio)
        draw_grid = random.random() < P_grid
        tick_step = random.randint(10, 16)

        #calling write_source_data_2() from bar_generator_vertical2 module
        write_source_data_2(data[i], f"./data/source/s_{i+3001}.png", fig_size_2, draw_grid, tick_step)
        postprocessing(f"./data/source/s_{i+3001}.png")   #saves the images in source folder within data folder
        
        #calling write_target_data_2() from bar_generator_vertical2 module
        write_circle_target_data_2(circle_data[i], f"./data/tactile/t_{i+3001}.png", fig_size_2, draw_grid, tick_step)
        maskgen(f"./data/tactile/t_{i+3001}.png")   #saves the images in tactile folder within data folder
        
        #calling write_target_data1_2() from bar_generator_vertical2 module
        write_circle_target_data1_2(circle_data[i], f"./data/tactile1/t_{i+3001}.png", fig_size_2, draw_grid, tick_step)
        print(f"./data/tactile1/t_{i+3001}.png")   #saves the images in tactile1 folder within data folder
        
        #similarly, calling methods from bar_generator_vertical2 module for 512x760 image dimensions
