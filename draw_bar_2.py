...
from bar_generator_2 import generate_data, write_source_data, write_circle_target_data1, serialize_data
...
with tf.device('/device:GPU:0'):
    ...
    for i in tqdm(range(len(data)), desc='bar charts: '):
        ...
        #fig_size = random.choices([[512,512], [512,512], [512,512]], weights=[.5, .25, .25])[0]
        fig_size = random.choices([[1024,512], [1024,512], [1024,512]], weights=[.5, .25, .25])[0]
        draw_grid = random.random() < P_grid
        tick_step = random.randint(10, 16)

        write_source_data(data[i], f"./data/source/s_{i+4001}.png", fig_size, draw_grid, tick_step)
        postprocessing(f"./data/source/s_{i+4001}.png")
        
        #write_circle_target_data(circle_data[i], f"./data/tactile/t_{i+4001}.png", fig_size, draw_grid, tick_step)
        #maskgen(f"./data/tactile/t_{i+4001}.png")
        
        write_circle_target_data1(circle_data[i], f"./data/tactile1/t_{i+4001}.png", fig_size, draw_grid, tick_step)
