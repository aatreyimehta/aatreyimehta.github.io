...
def generate_metadata(min_y = 0, max_y = 100):
    num_bars = 5 # np.random.randint(low=3, high=20+1) # Generate random number for number of bars, min: 3, max: 20
...
def generate_bar_widths(num_bars):
    one_width = np.random.randint(2) # Generate one width or different widths for bars
    if one_width:
        widths = np.random.uniform(0.05, 0.2) # reduce range to generate thinner bars
    else:
        widths = np.random.uniform(low=0.05, high=0.2, size=num_bars) # reduce range to generate thinner bars
    return widths

def generate_bar_gap():
    return np.round(np.random.uniform(0.5, 1.0), 2) # increase range to increase gap between bars

def generate_bar_group_gap():
    return np.round(np.random.uniform(0.5, 1.0), 2) # increase range to increase gap between groups of bars
...
# Draw source domain
def write_source_data(data, filepath, figsize=(600, 512), draw_grid=False, tick_step=10, grid_probability=global_grid_probability):
...
"""  
def write_circle_target_data(data, filepath, figsize=(672, 512), draw_grid=False, tick_step=10):
    fig = go.Figure()
...
"""
""" 
def write_circle_target_data(data, filepath, figsize=(512,1024), draw_grid=False, tick_step=10):
    fig = go.Figure()
    for i in range(len(data)):
        for j in range(len(data[i])):                              
            n = 1
...
def write_circle_target_data1(data, filepath, figsize=(672, 512), draw_grid=False, tick_step=10):
    fig = go.Figure()
    for i in range(len(data)):
        for j in range(len(data[i])):
            fig.add_trace(go.Scatter(x=data[i][j]["y"],
...
def write_circle_target_data1(data, filepath, figsize=(600, 512), draw_grid=False, tick_step=10, grid_probability=global_grid_probability):
