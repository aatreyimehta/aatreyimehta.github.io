...
with tf.device('/device:GPU:0'):
    ...
    for i in tqdm(range(len(data)), desc='bar charts: '):
        ...
        fig_tactile_size = random.choices([[256,256], [256,256], [256,256]], weights=[.5, .25, .25])[0]
        draw_grid = random.random() < P_grid
        tick_step = random.randint(10, 16)

        write_source_data(data[i], f"./data/source/s_{i+3001}.png", fig_size, draw_grid, tick_step)
        postprocessing(f"./data/source/s_{i+3001}.png")
        
        write_circle_target_data(circle_data[i], f"./data/tactile/t_{i+3001}.tiff", fig_tactile_size, draw_grid, tick_step)
        print(f"./data/tactile/t_{i+3001}.tiff")
