...
"""
def write_circle_target_data0(data, filepath, figsize=(512,1024), draw_grid=False, tick_step=10):
    fig = go.Figure()
...
"""    
def write_circle_target_data2(data, filepath, figsize=(512,1024), draw_grid=False, tick_step=10):
    fig = go.Figure()
    for i in range(len(data)):
        for j in range(len(data[i])):
            fig.add_trace(go.Scatter(x=data[i][j]["y"],
